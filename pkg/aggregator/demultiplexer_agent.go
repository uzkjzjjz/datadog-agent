...
func (d *AgentDemultiplexer) flushToSerializer(start time.Time, waitForSerializer bool) {
	...
	metrics.Serialize(
		...
		func(seriesSink metrics.SerieSink, sketchesSink metrics.SketchesSink) {
			...
			if len(d.lateMetrics) > 0 {
				log.Infof("---------------------------- will flush %d late metrics", len(d.lateMetrics))
				...
				for _, sample := range d.lateMetrics {
				...
				d.lateMetrics = d.lateMetrics[0:0]
				log.Infof("----------------- After the flush: %d", len(d.lateMetrics))
			}
...
func (d *AgentDemultiplexer) AddLateMetrics(samples metrics.MetricSampleBatch) {
	d.lateMetrics = append(d.lateMetrics, samples...)
}
