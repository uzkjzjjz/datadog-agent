...
func getMeta(ctx context.Context, hostnameData hostname.HostnameData) *Meta {
	...
	if config.Datadog.GetBool("hostname_force_config_as_canonical") &&
		hostnameData.Provider == hostname.HostnameProviderConfiguration {
	...
	if finalClusterName := kubelet.GetMetaClusterNameText(ctx, host); finalClusterName != "" {
