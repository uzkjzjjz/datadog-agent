...
//    https_process(t, b_data, b_len, GO);
    ...
    conn_tuple_t a = {0};
    bpf_map_delete_elem(&http_in_flight, &a);

    http_batch_key_t b = {0};
    bpf_map_delete_elem(&http_batches, &b);

    __u32 c = 0;
    bpf_map_delete_elem(&http_batch_state, &c);

    read_partial_call_key_t d = {0};
    bpf_map_delete_elem(&read_partial_calls, &d);

    uint32_t e = 0;
    bpf_map_delete_elem(&probe_data, &e);

    pid_fd_t f = {0};
    bpf_map_delete_elem(&sock_by_pid_fd, &f);
