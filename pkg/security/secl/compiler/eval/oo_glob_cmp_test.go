...
func TestOPOverrideGlobEquals(t *testing.T) {
	t.Run("no-match", func(t *testing.T) {
		...
		var ctx Context
		state := NewState(&testModel{}, "", nil, nil)

		e, err := GlobCmp.StringEquals(a, b, &Opts{}, state)
		assert.Empty(t, err)
		assert.False(t, e.Eval(&ctx).(bool))

		e, err = GlobCmp.StringEquals(b, a, &Opts{}, state)
		assert.Empty(t, err)
		assert.False(t, e.Eval(&ctx).(bool))
	})
...
func TestOPOverrideGlobContains(t *testing.T) {
	t.Run("no-match", func(t *testing.T) {
		...
		e, err := GlobCmp.StringValuesContains(a, b, &Opts{}, state)
		assert.Empty(t, err)
		assert.False(t, e.Eval(&ctx).(bool))
	})
...
func TestOPOverrideGlobArrayMatches(t *testing.T) {
	t.Run("no-match", func(t *testing.T) {
		...
		e, err := GlobCmp.StringArrayMatches(a, b, &Opts{}, state)
		assert.Empty(t, err)
...
func TestOPOverrideGlobArrayContains(t *testing.T) {
	t.Run("no-match", func(t *testing.T) {
		...
		e, err := GlobCmp.StringArrayContains(a, b, &Opts{}, state)
		assert.Empty(t, err)
		assert.False(t, e.Value)
	})
